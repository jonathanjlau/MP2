...
if __name__ == "__main__":
	...
	if ((len(sys.argv) < 2) | (len(sys.argv) > 3)):
		print 'Usage: python server.py config_file [command_file]'
		sys.exit()
	...
	max_delay['1'] = config['a']
	max_delay['2'] = config['b']
	max_delay['3'] = config['c']
	max_delay['4'] = config['d']

	# If a command file was given, add those commands into an array
	if len(sys.argv) ==3:
		input_commands = mp1util.read_file_lines(sys.argv[2])
